/* $RCSfile: xmlxvm.h $ $Date: 2021/07/27 19:05:34 $
...
/*---------------------------------------------------------------------------
    ...
    PURPOSE
        Resets the XSLT engine.

    DESCRIPTION
        Clears the engine of temporary memory from a prior run.
        Does not clear stack memory, which may have grown during the run.
        Stack memory is reused and likely similar sizes will be needed
        for subsequent runs. Check XmlXvmSize() to see if the stack memory
        has gotten too large to reuse or cache the engine instance.
...
/*--------------------------------------------------
 NAME
     XmlXvmGetSize
 PURPOSE
     Get the internal stack size (in bytes) for the xmlxvm
*/
size_t XmlXvmGetSize(xmlxvm *vm);
