...
/* 
    ...
    *OCIWideCharStrrchr
    ...
    OCIAsyncDisable
    OCIAsyncFdGet
    OCIAsyncCoroutineSubmit
    OCIAsyncWaitOne
    OCIAsyncWaitMany
    OCIAsyncCoroutineResume
    OCIAsyncCoroutineAbort
   ...
   MODIFIED   (MM/DD/YY)
   prramakr    10/16/21 - Backport prramakr_proj89657 from main
   ...
   vjitta      07/09/19 - Bug 30028971: Changing OCINonBlockingFdGet API
   prramakr    07/04/19 - proj 75757: OCI placeholder substitution
   kkverma     05/26/19 - Bug 18792392: Add OCIInputValidate()
   sriksure    05/08/19 - Bug 29750956: Add OCISodaCollTruncate()
   rkanodia    04/24/19 - Bug 29321709: add OCIServerDataLengthGet()
   vjitta      01/07/19 - Proj 32086: OCI NonBlocking Interface
                          Add OCINonBlockingEnable(), OCINonBlockingDisable()
                          and OCINonBlockingFdGet() and other OCIAsync API
...
sword   OCINonBlockingEnable(OCISvcCtx *svchp, OCIError *errhp, ub4 mode);

sword   OCINonBlockingDisable(OCISvcCtx *svchp, OCIError *errhp, ub4 mode);

sword   OCINonBlockingFdGet(void *hndlp, void *fd, ub4 *wouldBlockType,
                            OCIError *errhp, ub4 mode);
...
sword   OCIStmtPlaceholderSubstitute    (OCIStmt *stmtp, 
                                         const oratext *placeholderName, 
                                         ub4 placeholderNameLen, 
                                         oratext *subStr, ub4 subStrLen, 
                                         OCIError *errhp, ub4 mode);
...
/*--------------------- OCI Async Prototypes --------------------------------*/
sword OCIAsyncEnable(void     *hndlp, 
                     OCIAsync *asynchp, 
                     OCIError *errhp, 
                     ub4       mode);

sword OCIAsyncDisable (OCIAsync *asynchp,
                     OCIError *errhp,
                     ub4       mode);

sword OCIAsyncFdGet (OCIAsync *asynchp, 
                  void     *fd, 
                  ub4      *wouldBlkType, 
                  OCIError *errhp, 
                  ub4       mode);

sword OCIAsyncCoroutineSubmit(OCIAsync *asynchp, 
                              OCIAsyncCoroutine *proc, 
                              void *ctx, 
                              ub4 timeout, 
                              OCIError *errhp, 
                              ub4 mode);

sword OCIAsyncWaitOne(OCIAsync  *asynchp, 
                      ub4       timeout,
                      boolean   *isreadyp,
                      OCIError  *errhp, 
                      ub4       mode);

sword OCIAsyncWaitMany(OCIAsync **asynchp, 
                       ub4      count, 
                       ub4      timeout, 
                       OCIAsync **readyp,
                       ub4      *readycount, 
...
sword OCIAsyncCoroutineResume(OCIAsync *asynchp,
                              OCIError *errhp, 
                              ub4      mode);

sword OCIAsyncCoroutineAbort(OCIAsync *asynchp, 
                             OCIError *erhp, 
                             ub4      mode);
...
/*
   NAME
     OCIServerDataLengthGet - OCI Server data length get

   PARAMETERS
     hndlp         (IN)  define handle
     lengthValidp  (IN)  returns whether server data length was actually 
                         received for this define for the last column fetched
                         or being fetched. Note that this will be FALSE when
                         the server is not capable of sending the server data
                         length or if server data length is not supported for 
                         this define type/column type. Only when *lengthValidp
                         is TRUE, the caller should look at *lengthp
     lengthp       (IN)  returns server data length 
     errhp         (IN)  error handle
     mode          (IN)  mode

   DESCRIPTION
     Returns length of data (on the server) for the column fetched/being
     fetched for the define handle provided.

   RETURNS
       OCI Status

   NOTES
       This can also be used in a Dynamic Define callback to figure out the
       length of the data expected. Note that the length returned *lengthp is
       valid only if *lengthValidp is TRUE. 
*/

sword OCIServerDataLengthGet(void     *hndlp,
                             boolean  *lengthValidp,
                             ub8      *lengthp,
                             OCIError *errhp,
                             ub4       mode);

sword OCIInputValidate(oratext    *str, 
                       ub4         strl, 
                       OCIError   *errhp, 
                       ub4         mode);
