/* Copyright (c) 1993, 2019, Oracle and/or its affiliates. 
All rights reserved.*/
...
/*
  ...
  PUBLIC FUNCTIONS
    ...
    OCIDate - OCI functions to manipulate OCI Date
      ...
      OCIDateAddDaysSeconds - OCIDate ADd or subtract Days and Seconds
  ...
  MODIFIED
    ssinabhi   05/16/19 - Bug 29474868: add OCIDateDaysSecondsBetween
    anjs       11/09/18 - Bug 28859071: add OCIDateAddDaysSeconds
...
sword OCIDateAddDaysSeconds(  OCIError *err, const OCIDate *date, sb4 num_days,
                              sb4 num_seconds, OCIDate *result  );
/*
   NAME: OCIDateAddDaysSeconds - OCIDate ADd or subtract Days and Seconds
   PARAMETERS:
        err (IN/OUT) - error handle. If there is an error, it is
                recorded in 'err' and this function returns OCI_ERROR.
                The error recorded in 'err' can be retrieved by calling
                OCIErrorGet().
        date (IN) - 'num_days', 'num_seconds' added or subtracted from 'date'
        num_days (IN) - number of days to be added or subtracted
                (a negative value will be subtracted)
        num_seconds (IN) - number of seconds to be added or subtracted
                (a negative value will be subtracted)
        result (IN/OUT) - result of adding or subtracting to 'date'
   DESCRIPTION:
        The function OCIDateAddDaysSeconds adds or subtracts num_days and
        num_seconds from the date 'date'.
   RETURNS:
        OCI_SUCCESS if the function completes successfully.
        OCI_INVALID_HANDLE if 'err' is NULL.
        OCI_ERROR if
...
sword OCIDateDaysSecondsBetween(    OCIError *err, const OCIDate *date1, 
                                    const OCIDate *date2, sb4 *num_days, 
                                    sb4 *num_seconds    );
/*
  NAME: OCIDateDaysSecondsBetween - OCIDate get number of Days and Seconds 
                                    Between two dates.
  PARAMETERS:
        err         (IN/OUT) - error handle. If there is an error, it is
        ...
        date1, date2    (IN) - input dates.
        num_days       (OUT) - number of days between date1 and date2
                               (date1 - date2). Returns negative value
                               when date1 is earlier than date2.
        num_seconds    (OUT) - number of residual seconds (after maximum 
                               possible full days between the dates has been 
                               subtracted) between date1 and date2.Returns
                               negative value when date1 is earlier than date2.
   
   DESCRIPTION:
        The function OCIDateDaysSecondsBetween returns the number of days and
